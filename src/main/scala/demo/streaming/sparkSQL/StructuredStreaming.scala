package demo.streaming.sparkSQL

import java.text.SimpleDateFormat
import java.util.Locale
import java.util.regex.{Matcher, Pattern}

import demo.streaming.Utilities.{apacheLogPattern, setupLogging}
...
object StructuredStreaming {
   ...
   def main(args: Array[String]) {
      ...
      val spark = SparkSession
        ...
        .master("local[*]")
        .config("spark.sql.streaming.checkpointLocation", "/Users/sromero/Development/SparkScala/checkpoint/")
        .getOrCreate()
        
      setupLogging()
        
      // Create a stream of text files dumped into the logs directory
      val rawData = spark.readStream.text("/Users/sromero/Development/SparkScala/logs")
